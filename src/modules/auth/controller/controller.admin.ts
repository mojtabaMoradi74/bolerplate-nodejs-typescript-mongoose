...
class AuthController {
    ...
    async Login(req: Request, res: Response): Promise<any> {
        ...
        if (!validate) {
            throw new BadRequestException(
                invalidError({ key: 'password', ns: "translate" })
            );
        } else if (user.status !== STATUS.ACTIVE) {
            throw new ForbiddenException(ActiveError({ key: 'user' }));
        } else if (user.type !== "ADMIN") {
            throw new ForbiddenException(invalidError({ key: 'user' }));
    ...
    async Refresh(req: Request, res: Response) {
        const { user, tokenDecrypted, tokenEncrypted } = req
        const refreshToken = tokenEncrypted
        // console.log({ user, tokenDecrypted, tokenEncrypted });
        ...
        try {
            const token: Record<string, any> = await this.tokenService.refresh({
                user: tokenDecrypted._id,
                rememberMe: tokenDecrypted.rememberMe,
                loginDate: tokenDecrypted.loginDate,
                refreshToken,
                tokenDecrypted,
                tokenEncrypted
            })
