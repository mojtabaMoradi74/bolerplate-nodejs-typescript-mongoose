...
export class HelperDateService {
    ...
    constructor() {
        this.timezone = configService.app.timezone;
    }
    ...
    isExpired(expired: Date): boolean {
        const today: Date = this.create();
        const expiredConvert: Date = this.create({
            date: expired,
        });

        if (today > expiredConvert) return true;
